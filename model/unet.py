...
class Conv2d(torch.nn.Module):
    def __init__(
        ...
        # weight is also the kernel.
        # [cout, cin, kernal, kernal]
        self.weight = (
            torch.nn.Parameter(
...
class PositionalEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        freqs = torch.arange( # [0, 64]
            start=0, end=self.num_channels // 2, dtype=torch.float64, device=x.device
        )
        freqs = freqs / (self.num_channels // 2 - (1 if self.endpoint else 0)) # [0, 1]
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        dropout             = 0.2,         # Dropout probability of intermediate activations.
        ...
        **kwargs
        ...
        if embedding_type == 'positional':
            ...
            self.map_noise = FourierEmbedding(num_channels=noise_channels, **embedding_kwargs)

        self.map_layer0 = Linear(in_features=noise_channels, out_features=emb_channels, **init)
        self.map_layer1 = Linear(in_features=emb_channels, out_features=emb_channels//2, **init)
        self.map_label = Linear(in_features=label_dim, out_features=noise_channels*4, **init) if label_dim else None
 
        self.s_embed = s_embed
        if s_embed:
            if embedding_type == "positional":
                self.map_noise_s = PositionalEmbedding(num_channels=noise_channels, endpoint=True)
            elif embedding_type == "fourier":
                self.map_noise_s = FourierEmbedding(num_channels=noise_channels, **embedding_kwargs)

            self.map_layer0_s = Linear(in_features=noise_channels, out_features=emb_channels, **init)
            self.map_layer1_s = Linear(in_features=emb_channels, out_features=emb_channels//2, **init)
